...
export class DialerComponent implements OnInit {
  ...
  onCall():void {
    if (this.call) {
      ...
      } else if (this.numberCtrl.value.substring(0, 1) == "/") {
  ...
  handleCallEvent(state:string):void{
    this.zone.run(() => {
      ...
      if (state == "ended") {
        ...
        this.call.getLocalMedia().setElement(this.outgoingMedia.nativeElement, true).subscribe((size) => this.updateSize(size))
      }
